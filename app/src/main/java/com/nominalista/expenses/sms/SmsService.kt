...
class SmsService : IntentService("com.nominalista.expenses.sms.SmsService") {
    ...
    private fun getRule(context: Context, message: String): Keyword {
        val dataStore = context.application.localDataStore
        val keywords = dataStore.getKeywords()
                .subscribeOn(Schedulers.io())
                .blockingGet()
        return keywords.filter { message.contains(it.name, ignoreCase = true) }.first()
    }

    fun getAmount(message: String, rule: Keyword): Double {
